...
1) zones: 각 정보 구역을 분류한다.
   ...
   - 중요: 하나의 긴 이미지 안에 여러 정보(예: 사이즈표, 소재, 디테일)가 함께 있을 수 있다.
     이 경우 각각을 별도의 zone 으로 나누고, verticalRatio 로 그 이미지 안에서의 세로 위치(0.0~1.0)를 표시한다.
     예: 한 이미지에 위쪽에 소재 설명, 아래쪽에 사이즈표가 있으면
         소재 zone(verticalRatio 0.2), 사이즈 zone(verticalRatio 0.6) 처럼 서로 다른 값을 준다.
   - verticalRatio 는 그 이미지 안에서 해당 정보가 "시작하는" 세로 위치 비율이다. 최대한 정확히 추정한다.
