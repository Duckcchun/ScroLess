...
쇼핑몰의 긴 "통이미지" 상세페이지에서, 원하는 정보(사이즈·착용샷·소재·세탁)로 **즉시 이동**하게 해주는 크롬 확장 프로그램입니다.

긴 상세이미지를 AI(Google Gemini) 비전 모델로 분석해 정보 구역을 인식하고, 화면 옆 스마트 인덱스로 원하는 곳에 바로 점프하게 해줍니다.

## 어떻게 동작하나

```
[크롬 확장] 상세이미지 수집(노이즈 제외 + lazy 대응)
      │  이미지 URL 전송
      ▼
[백엔드/Render] Gemini 분석 → { zones, chips } JSON (+ 캐싱)
      │  결과 반환
      ▼
[크롬 확장] 스마트 인덱스 · 요약 칩 · 접근성 텍스트 렌더링
```

- 확장은 상세이미지의 URL만 백엔드로 보냅니다. 개인정보는 다루지 않습니다.
- 백엔드는 배포되어 있어(`https://scroless-backend.onrender.com`), 확장만 설치하면 바로 동작합니다.
- API 키는 백엔드에서만 관리되어 클라이언트에 노출되지 않습니다.
